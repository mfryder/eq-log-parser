...
class Battle {
    ...
    discoverInformationFromBattleText(battleText){
        this.discoverPlayerName(_.cloneDeep(battleText));
        this.discoverTargetName(_.cloneDeep(battleText));
        this.discoverTargetLevel(_.cloneDeep(battleText));
        this.discoverPlayerXPEarned(_.cloneDeep(battleText));
        this.discoverMoneyEarned(_.cloneDeep(battleText));
        this.discoverStartTime(_.cloneDeep(battleText));
        this.discoverEndTime(_.cloneDeep(battleText));
        this.discoverLoot(_.cloneDeep(battleText));
        this.discoverLocation(_.cloneDeep(battleText));
        this.discoverZone(_.cloneDeep(battleText));
        this.discoverContinent(_.cloneDeep(battleText));
        this.discoverServer(_.cloneDeep(battleText));
        this.discoverPlayerDPS(_.cloneDeep(battleText));
        this.discoverTargetDPS(_.cloneDeep(battleText));
        this.discoverPlayerMPConsumption(_.cloneDeep(battleText));
        this.discoverPlayerVictory(_.cloneDeep(battleText));
    }
    ...
    discoverPlayerMPConsumption(battleText){
        //need cleaner approach. Perhaps just detect spell casting... and reference spell mana cost??
    ...
    discoverPlayerXPEarned(battleText){
        ...
        if(experienceArray.length > 0){
            this.setPlayerXPEarned(true);
        }else{
            this.setPlayerXPEarned(false);
        }
    }
    ...
    discoverPlayerVictory(battleText){
        let victoryLine = battleText[battleText.length -1];
        logger.info(victoryLine)
        if(victoryLine.indexOf("Fight Ended with") > -1 && victoryLine.indexOf("killed") >-1){
    ...
    discoverZone(battleText){
        let zone = battleText.filter(text => (text.indexOf("There is") > -1 || text.indexOf("There are") > -1) && text.indexOf("in") > -1)
                                .map(text => {
